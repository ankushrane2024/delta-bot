...
if live_tab_start != -1 and live_mode_tab_start != -1:
    ...
    for i, line in enumerate(lines):
        ...
        if in_live_tab:
            for cid in common_ids:
                # Do not prefix if it already has live-
    ...
    for i, line in enumerate(lines):
        ...
        if 'async function runLiveTestOrder()' in line:
            in_live_js = False
            break
    ...
    with open('c:/Users/AnkushR/Downloads/Delta_BTC_Options_Bot/Delta_BTC_Options_Bot/templates/dashboard.html', 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))
    print(f"Safe fixed duplicated IDs globally in the Live tab and fetchLiveStatus().")
else:
